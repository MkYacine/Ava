...
def initialize_session_state():
    ...
    if 'pipeline_stage_recording' not in st.session_state:
        st.session_state.pipeline_stage_recording = 'start'
    if 'pipeline_stage_analysis' not in st.session_state:
        st.session_state.pipeline_stage_analysis = 'upload_transcript'
...
if check_password():
    ...
    # Create two tabs for the different pipeline parts
    tab1, tab2 = st.tabs(["Call Recording Pipeline", "Analysis Pipeline"])

    with tab1:
        # Original pipeline code for call recording and transcription# Call this function at the start of your app    # Main pipeline
        if st.session_state.pipeline_stage_recording == 'start':
                ...
                if forward_number and to_number:
                    try:
                        ...
                        if response.status_code == 200:
                            ...
                            st.session_state.pipeline_stage_recording = 'wait_for_call'
                        else:
        ...
        if st.session_state.pipeline_stage_recording == 'wait_for_call':
            ...
            if call.status in ['completed', 'failed', 'busy', 'no-answer', 'canceled']:
                if call.status == 'completed':
                    ...
                    st.session_state.pipeline_stage_recording = 'process_recording'
                else:
        ...
        if st.session_state.pipeline_stage_recording == 'process_recording':
            ...
            while attempt < max_attempts:
                ...
                if recordings:
                    ...
                    if response.status_code == 200:
                        ...
                        st.session_state.pipeline_stage_recording = 'transcribe'
                        break
                    else:
        ...
        if st.session_state.pipeline_stage_recording == 'transcribe':
    ...
    with tab2:
        st.header("Transcript Analysis Pipeline")
        # New upload stage
        if st.session_state.pipeline_stage_analysis == 'upload_transcript':
            ...
            if uploaded_file is not None:
                try:
                    ...
                    # Show sample transcript format
                    with st.expander("View sample transcript format"):
                        st.code(uploaded_file)
                    
                    if st.button("Start Analysis"):
                        st.session_state.pipeline_stage_analysis = 'generate_ai_response'
                        st.rerun()
                except Exception as e:
        ...
        if st.session_state.pipeline_stage_analysis == 'generate_ai_response':
            ...
            try:
                ...
                st.session_state.pipeline_stage_analysis = 'validate_form'
            except Exception as e:
        ...
        if st.session_state.pipeline_stage_analysis == 'validate_form':
            st.header("Validate Form")
            logger.log_text("Starting form validation", severity='INFO')
            # Load mock audio files (needed for validation stage)
            ...
            if st.session_state.cleaned_form:
                ...
                if not st.session_state.issues:
                    ...
                    st.session_state.pipeline_stage_analysis = 'generate_pdf'
        ...
        if st.session_state.pipeline_stage_analysis == 'generate_pdf':
            ...
            try:
                ...
                st.session_state.pipeline_stage_analysis = 'salesforce_integration'
            except Exception as e:
        ...
        if st.session_state.pipeline_stage_analysis == 'salesforce_integration':
            ...
            try:
                ...
                st.session_state.pipeline_stage_analysis = 'complete'
            except Exception as e:
                st.error(f"An error occurred during Salesforce integration: {str(e)}")
                st.session_state.pipeline_stage_analysis = 'upload_transcript'

        if st.session_state.pipeline_stage_analysis == 'complete':
